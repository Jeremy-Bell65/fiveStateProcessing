...
//    processorAvailable = true;
    ...
    // Current process
    list<Process>::iterator processes = processList.begin();
    Process currentProcess = *processes;
    // Do main loop until finished
    bool finished = false;

    //keep running the loop until all processes have been added and have run to completion
    while(processMgmt.moreProcessesComing() || !finished)
    {
        ...
        // Init a iterator to check for interrupts
        list<Process>::iterator interrupt = processList.begin();
        Process interruptChecker = *interrupt;
        ...
        }else if (currentProcess.state == processing){// If process currently running, continue
          // Check for IO event
          if (currentProcess.processorTime == currentProcess.ioEvents.begin()->time){
            ioModule.submitIORequest(time, currentProcess.ioEvents.front(), currentProcess);

            // Remove io event
            currentProcess.ioEvents.pop_front();

            currentProcess.state = blocked;
            stepAction = ioRequest;
          }else if (currentProcess.processorTime < currentProcess.reqProcessorTime){// If no IO event, run normally
            currentProcess.processorTime++;
            stepAction = continueRun;
          }else{// Once finished end task
            currentProcess.state = done;
            currentProcess.doneTime = time;
            stepAction = complete;
          }
        }else{
          processes++;
          currentProcess = *processes;
          // Iterate through process list until it finds a process that needs handling
          bool checkingProcesses = true;

          while (checkingProcesses){
            switch(currentProcess.state){// Note case of processing not listed as it is handled above
              case ready:
                // Begin processing
                currentProcess.state = processing;
                currentProcess.processorTime++;
                stepAction = beginRun;
            ...
            // Start processes at beginning
            if (processes == processList.end()){
              processes = processList.begin();
            }
            currentProcess = *processes;
